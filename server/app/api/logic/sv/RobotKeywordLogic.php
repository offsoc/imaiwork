 ...
 * @author Qasim
 */
class RobotKeywordLogic extends SvBaseLogic
{
    ...
    public static function addRobotKeyword(array $params)
    {
        try {
            ...
            // 检查关键词是否已添加
            $keyword = SvRobotKeyword::where('user_id', self::$uid)->where('robot_id', $params['robot_id'])->where('keyword', $params['keyword'])->findOrEmpty();
    ...
    public static function updateRobotKeyword(array $params)
    {
        ...
        try {
            // 检查机器人是否存在
            $robot = SvRobotKeyword::where('id', $params['id'])->where('user_id', self::$uid)->findOrEmpty();
            if ($robot->isEmpty()) {
    ...
    public static function importRobotKeyword(array $params)
    {
        try {
            ...
            foreach ($content as $key => $value) {
                ...
                // 是否存在
                $keyword = SvRobotKeyword::where('user_id', self::$uid)->where('robot_id', $fields['robot_id'])->where('keyword', $fields['keyword'])->findOrEmpty();
                if (!$keyword->isEmpty()) {
