...
use app\common\model\wechat\AiWechatLabel;
 ...
 * @author Qasim
 */
class LabelLogic extends WechatBaseLogic
{
    ...
    public static function addLabel(array $params)
    {
        ...
        try {
            ...
            $label = AiWechatLabel::where('user_id', self::$uid)->where('wechat_id', $params['wechat_id'])->where('label_name', $params['label_name'])->limit(1)->findOrEmpty();
            if ($label->isEmpty()) {
                $label = AiWechatLabel::create($params);
            } else {
                AiWechatLabel::where('id', $label->id)->update($params);
            }
    ...
    public static function importLabel(array $params)
    {
        try {
            ...
            foreach ($content as $key => $value) {
                ...
                // 是否存在
                $keyword = AiWechatLabel::where('user_id', self::$uid)->where('wechat_id', $fields['wechat_id'])->where('keyword', $fields['keyword'])->findOrEmpty();
                if (!$keyword->isEmpty()) {
                    AiWechatLabel::where('id', $keyword->id)->update($fields);
                } else {
                    AiWechatLabel::create($fields);
                }
            }
            return true;
        } catch (\Exception $e) {
