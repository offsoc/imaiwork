...
class PublishDetailLists extends BaseApiDataLists implements ListsSearchInterface
{
    ...
    {
        return [
            '=' => ['ps.status', 'ps.material_type', 'ps.device_code', 'ps.account_type','ps.publish_id','ps.account'],
    ...
    public function lists(): array
    {
        ...
        $this->searchWhere[] = ['ps.task_type', '=',  $this->request->get('task_type', 3)];
        return  SvPublishSettingDetail::alias('ps')
            ...
            ->select()->each(function ($item) {
                ...
                if( $item['material_tag']){
                    $item['material_tag'] = explode(',', $item['material_tag']);
                }
            })
            ->toArray();
    }
