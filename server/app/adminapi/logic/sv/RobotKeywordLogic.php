 ...
 * @author Qasim
 */
class RobotKeywordLogic extends BaseLogic
{
    ...
    public static function addRobotKeyword(array $params)
    {
        try {
            ...
            // 检查机器人是否存在
            $robot = KbRobot::where('id', $params['robot_id'])->where('user_id', 0)->findOrEmpty();
            if ($robot->isEmpty()) {
            ...
            // 检查关键词是否已添加
            $keyword = SvRobotKeyword::where('user_id', 0)->where('robot_id', $params['robot_id'])->where('keyword', $params['keyword'])->findOrEmpty();
            ...
            // 添加
            $robot = SvRobotKeyword::create($params);
    ...
    public static function updateRobotKeyword(array $params)
    {
        ...
        try {
            // 检查机器人是否存在
            $robot = SvRobotKeyword::where('id', $params['id'])->where('user_id', 0)->findOrEmpty();
            if ($robot->isEmpty()) {
            ...
            // 更新
            SvRobotKeyword::where('id', $robot->id)->update($params);
    ...
    public static function importRobotKeyword(array $params)
    {
        try {
            ...
            foreach ($content as $key => $value) {
                ...
                // 是否存在
                $keyword = SvRobotKeyword::where('user_id', 0)->where('robot_id', $fields['robot_id'])->where('keyword', $fields['keyword'])->findOrEmpty();
                if (!$keyword->isEmpty()) {
                    SvRobotKeyword::where('id', $keyword->id)->update($fields);
                } else {
                    SvRobotKeyword::create($fields);
                }
            }
            return true;
        } catch (\Exception $e) {
