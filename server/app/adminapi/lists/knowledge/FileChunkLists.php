 ...
 * 形象
 */
class FileChunkLists extends BaseAdminDataLists implements ListsSearchInterface
{
    ...
    { 
        return KnowledgeFileSlice::alias('k')
                ...
                ->when($this->request->get('id'), function ($query) {
                    $query->where('k.rid', '=', $this->request->get('id'));
                })
                ...
                ->when($this->request->get('status'), function ($query) {
                    $query->where('k.status', '=', $this->request->get('status'));
                })
