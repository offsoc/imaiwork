...
class XhsSocketService
{
    ...
    {   
        ...
        try {
            ...
            if($uid) {
                ...
                $handler = match($type) {
                    1 => new \app\common\workerman\xhs\handlers\DeviceHandler($this), #获取设备信息、状态
                    2 => new \app\common\workerman\xhs\handlers\UserHandler($this), #小红书用户信息
                    3 => new \app\common\workerman\xhs\handlers\PrivateMessageHandler($this), #私信列表信息
                    4 => new \app\common\workerman\xhs\handlers\InteractiveMessageHandler($this), #已发布内容状态、收藏、点赞
                    7 => new \app\common\workerman\xhs\handlers\CardHandler($this), #发送私信消息
                    10 => new \app\common\workerman\xhs\handlers\CardHandler($this), #发送卡片
                    ...
                    'getPrivateMessageList' => new \app\common\workerman\xhs\handlers\PrivateMessageHandler($this), #web端获取私信列表
                    'sendPrivateMessage' => new \app\common\workerman\xhs\handlers\MessageHandler($this), #最新私信消息
                    'getCards' => new \app\common\workerman\xhs\handlers\CardHandler($this), #web端获取名片列表
                    'getPostList' => new \app\common\workerman\xhs\handlers\InteractiveMessageHandler($this), #已发布内容状态、收藏、点赞
                    'initCheck' => new \app\common\workerman\xhs\handlers\CheckInitHandler($this), #设备初始化检测
                    'sendCard' => new \app\common\workerman\xhs\handlers\CardHandler($this), #web端获取名片列表
                    'ping' => new \app\common\workerman\xhs\handlers\HeartBeatHandler($this),#心跳
                    // ...其他case对应的handler...
                    default => new \app\common\workerman\xhs\handlers\DefaultHandler($this)
                };
    ...
    private function _runCrontab(){
        // $handler = new \app\common\workerman\xhs\handlers\CrontabHandler($this);
        // $handler->runing();
    }
