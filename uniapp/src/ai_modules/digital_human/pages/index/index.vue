<template>
	<view class="min-h-screen bg-white pb-[100rpx]">
		...
		<view class="relative w-full h-[642rpx]">
			<image
				src="@/ai_modules/digital_human/static/images/home/bg.png"
...
import { digitalHumanLists, retryVideo } from "@/api/digital_human";
...
const loopLists = async () => {
	loopTimer.value = setTimeout(() => {
		const isLoading = pager.lists.some(
			(item: any) => item.status != 1 && item.status != 2
		);
		if (isLoading) {
			getLists();
			loopLists();
