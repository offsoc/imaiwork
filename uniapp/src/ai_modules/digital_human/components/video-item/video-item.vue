	...
	<view class="w-full h-full relative flex flex-col">
		<view class="grow min-h-0 relative overflow-hidden rounded-lg bg-black">
			<view
				...
				<view
					class="flex justify-center items-center h-[85%] w-[70%] relative z-10">
					<image
						:src="item.pic"
						class="w-full mx-auto h-full rounded-lg"
						mode="aspectFill"></image>
					<view
						v-if="item.automatic_clip == 1"
						class="absolute left-2 top-2 text-[20rpx] text-white"
						>AI剪辑</view
					>
			...
			<view class="absolute top-0 left-0 z-50 w-full h-full">
				<template v-if="item.status == 1">
					...
					<view
						...
						<template
							v-if="
								item.clip_status == 1 || item.clip_status == 2
							">
							AI智能剪辑中...
						</template>
				...
				<template v-else>
					<view
						class="bg-[#0000005E] w-full h-full flex flex-col items-center justify-center gap-2">
						<template class="" v-if="item.status == 2">
							...
							<view
								class="text-[#ffffff80] text-center text-[22rpx]">
								（请检查训练的视频文件）
							</view>
						</template>
						<template v-else>
