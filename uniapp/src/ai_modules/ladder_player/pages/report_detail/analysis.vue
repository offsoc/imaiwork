			...
			<view
				...
				<view class="bg-white rounded-xl p-3">
					<view>
						...
						<view>
							<text class="text-primary font-bold text-[72rpx]">{{
				...
				<view class="mt-4">
					<u-sticky :offset-top="stickyTop" bg-color="transparent">
						<view
							class="flex gap-2 bg-[#7E9EF8] rounded-[16rpx] p-1">
							<view
				...
				<view
					...
					<view class="flex items-center -mt-2">
						...
						<view class="ml-[64rpx]">
							<text class="text-primary font-bold text-[48rpx]">{{
								item.score || 0
							}}</text>
		...
		uni.createIntersectionObserver(proxy, {
			...
			.observe(".column", (res: any) => {
				if (res && res.intersectionRatio == 1) {
					const visibleIndex = indicator.value.findIndex(
						(item) => item.id === res.id
					);
