    ...
    <view class="h-screen flex flex-col relative">
        ...
        <view class="grow min-h-0 relative z-10">
            <view class="h-full flex">
                <view class="w-[248rpx] bg-white h-full flex flex-col overflow-hidden flex-shrink-0">
                    <view class="grow">
                        <scroll-view scroll-y>
                            <view
                                ...
                                :key="index"
                                :style="{
                                    fontWeight: robotCateIndex == index ? 'bold' : 'normal',
                                    color: robotCateIndex == index ? '#0065FB' : '#383838',
                                }"
                ...
                <view class="flex-1 flex flex-col min-h-0 overflow-hidden bg-[#F6F8FE]">
                    <view v-if="getRobotSubCate.length" class="w-full mt-4 px-2">
                        <scroll-view scroll-x class="w-full">
                            <view class="flex gap-2">
                                <view
                                    ...
                                    :key="index"
                                    :style="{
                                        fontWeight: robotSubCateIndex == index ? 'bold' : 'normal',
                                        color: robotSubCateIndex == index ? '#ffffff' : '#383838',
                                        backgroundColor: robotSubCateIndex == index ? '#0065FB' : '#EDEBFC',
                                    }"
        ...
        transformData: (data) => {
            if (data.lists.length) {
                ...
                if (robotSubCateIndex.value == 0) {
                    ...
                    queryParams.scene_id = sub_list.length ? sub_list[robotSubCateIndex.value].id : "";
                }
...
const getRobotSubCate = computed(() => {
    return [{ name: "全部", id: "" }].concat(optionsData.robotCate[robotCateIndex.value]?.sub_list || []);
});
...
const handleRobotSubCate = (index: number) => {
    ...
    queryParams.scene_id = robotSubCateIndex.value == 0 ? getRobotCateId.value : getRobotSubCate.value[index].id;
    pagingRef.value?.reload();
};
