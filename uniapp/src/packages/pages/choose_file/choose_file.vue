<template>
	<view class="h-screen flex flex-col bg-[#F6F7F9] px-[50rpx]">
		<view class="grow flex flex-col items-center mt-[10vh]">
			<view>
				<image
					src="@/packages/static/images/common/file_bg.png"
					class=""
					mode="aspectFit"></image>
			</view>
			<view class="text-center text-[#6D7074]">
				支持PDF、文档、表格、图片等格式，最多上传{{
					fileLimit
				}}个文件，每个文件不超过20M，图片暂时只能上传一张
			</view>
			<view class="w-[80%] mt-[80rpx] flex flex-col gap-4">
				<view
					...
					<u-icon
						name="/static/images/icons/weixin.svg"
						:size="32"></u-icon>
...
const fileLimit = ref<number>(9);
const sumImage = ref<number>(1);
...
const openFile = async (type: string) => {
	...
	if (type === "record") {
		filesResult = await chooseFile({
			type: "file",
			count: fileLimit.value - (fileLists.value.length + sumImage.value),
		});
	} else if (type === "album") {
		filesResult = await chooseFile({
			type: "image",
...
const upload = (files: FileData[]): Promise<void> => {
	...
	return new Promise((resolve) => {
		...
		const run = async () => {
			...
			const currentIndex = fileLists.value.findIndex(
				(item) => item.path === fileItem.path
			);

			try {
				const { uri, id }: any = await uploadGPTFile(
					{
