<template>
    <view class="h-full flex flex-col">
        <view class="flex flex-col items-center pt-[20vh] pb-[40rpx]">
            <image :src="appStore.getWebsiteConfig.pc_logo" mode="aspectFit" class="h-[88rpx]"></image>
        ...
        <view
            ...
            <div class="flex justify-around mt-[40rpx] px-[80rpx]">
                <div
                    ...
                    <u-icon name="/static/images/icons/icon_kj.png" :size="80" />
                    <div class="text-sm mt-[10px]">快捷登录</div>
                </div>
                <div
                    class="flex flex-col items-center"
                    v-if="hasMobileLogin && !isMobileLogin"
                    @click="changeLoginWay(LoginWayEnum.MOBILE)">
                    <u-icon name="/static/images/icons/icon_phone.png" :size="80" />
...
import { OALogin, login, mnpLogin as mnpLoginApi, updateUser, uninAppLogin, mnpAuthBind } from "@/api/account";
...
const { lockFn: wxLoginLock, isLock } = useLockFn(async (res?: any) => {
    let data: any = null;
    try {
        // #ifdef H5
        data = await oaLogin();
