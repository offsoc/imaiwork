<template>
	<view
		class="relative overflow-hidden rounded-xl bg-[#F8F8F8] w-full h-full">
		<view
			v-if="isImage(item.url)"
			class="w-full h-full flex items-center justify-center">
			<view
				class="h-full w-full min-w-[86rpx] min-h-[86rpx] p-1.5"
		...
		<view class="p-2 min-w-40" v-else>
			<view class="flex flex-row items-center gap-2">
				<view
					...
					<image
						:src="getFileTypeValue.icon"
						class="w-[50rpx] h-[50rpx]"
						color="#ffffff"></image>
				</view>
				<view class="overflow-hidden">
					<view class="text-sm line-clamp-1">
						{{ item.name }}
					</view>
	...
	<view
		class="absolute right-0 -top-1 -translate-y-1/2 translate-x-1/2 z-10"
		@click="del(index)"
		v-if="showDel">
		<view
			class="bg-[rgba(0,0,0,0.5)] w-[24rpx] h-[24rpx] rounded-full flex items-center justify-center">
