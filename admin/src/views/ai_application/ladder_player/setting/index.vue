			...
			<el-form
				...
				<el-form-item label="音色管理" prop="voice">
					<div class="flex flex-wrap gap-2">
						<div
							v-for="(item, index) in formData.voice"
							:key="index"
							class="">
...
const rules = {
	avatars: [
		{ required: true, message: "请选择随机头像素材库", trigger: "blur" },
	],
...
const handleLanguageAdd = () => {
	// 判断上个元素是否为空, 否则提示
	if (
		!formData.value.language[formData.value.language.length - 1] &&
		formData.value.language.length > 0
	) {
