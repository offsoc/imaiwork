			...
			<div class="mt-3">
				<div class="flex flex-wrap gap-3" v-if="getTags">
					<div
						...
						class="px-3 py-1 bg-[#E8F0FF] text-primary rounded-md">
						{{ tag }}
					</div>
				</div>
		...
		<div class="grow min-h-0 flex flex-col">
			<div>
				<el-tabs v-model="activeTab">
					<el-tab-pane label="章节速览" name="1"></el-tab-pane>
					<el-tab-pane label="发言总结" name="2"></el-tab-pane>
					<el-tab-pane label="问答回顾" name="3"></el-tab-pane>
				</el-tabs>
			</div>
					...
					<div class="p-4">
								...
								<div
									...
									<div class="relative">
										<div
											...
											<div
												class="w-2 h-2 rounded-full bg-[#000000]"></div>
										</div>
										<div
											class="h-full border-r border-dashed border-primary absolute right-1"
											v-if="
												index < getSections.length - 1
											"></div>
									</div>
									<div
										class="p-6 bg-primary-light-8 rounded-md">
										<div>
											{{ item.Headline }}
										</div>
										<div
											class="mt-2 text-xs text-[#63647C]">
											{{ item.Summary }}
										</div>
									</div>
								</div>
							</div>
							<div class="text-[#c8cad9]" v-else>
								暂无章节速览
							</div>
						</template>
						<template v-if="activeTab == '2'">
							<div
								...
								v-if="getConversational">
								<div
									class="flex gap-3 bg-[#E8F0FF] p-3 rounded-lg"
									v-for="item in getConversational"
									:key="item">
									<div
										class="flex-shrink-0 w-[65px] min-w-[65px] flex flex-col gap-2 items-center">
										<img
											class="w-[28px] h-[28px]"
											:src="
												avatarList[item.SpeakerId - 1]
											" />
										<span class="text-[#585A73] text-xs">{{
											item.SpeakerName
										}}</span>
									</div>
									<div
										class="text-[#585A73] text-xs leading-[20px]">
										{{ item.Summary }}
									</div>
								</div>
							</div>
						...
						<template v-if="activeTab == '3'">
							<div class="flex flex-col gap-3" v-if="getQa">
								<div
									...
									class="rounded flex overflow-hidden">
									<div
										class="bg-[#DEE5FA] w-[48px] flex-shrink-0 flex items-center justify-center">
										<span
											class="text-xl font-bold text-primary"
											>Q</span
										>
										<span
											class="text-[10px] font-bold text-[#FF8D1A]"
											>A</span
										>
									</div>
...
.loading-box {
	...
	background: url("../../_assets/images/loading_bg.png") no-repeat center
		center;
