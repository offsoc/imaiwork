<template>
					...
					<ElFormItem>
						<div class="w-full">
							...
							<div
								...
								v-if="
									generateType ===
									GenerateTypeEnum.PLATFORM_CHOICE
								">
								<div
									class="flex items-center gap-2 text-[11px] px-[6px] h-[26px]">
									<div
										v-for="item in optionsData.template
											.categories"
								...
								<div
									class="mt-[6px] h-[232px] bg-app-bg-3 rounded-md shadow-[0_0_0_1px_#2A2A2A]">
									<ElScrollbar>
										<div
											class="grid grid-cols-3 gap-2 p-[6px]">
											<div
												v-for="(
													item, index
												) in getTemplateOptions"
							...
							<div
								...
								v-if="
									generateType ===
									GenerateTypeEnum.CREATIVE_DESCRIPTION
								">
								<div
								...
								<ElButton
									...
									@click="
										handleGeneratePrompt(
											CopywritingTypeEnum.AI_GOODS_IMAGE
										)
									">
									生成提示词
								</ElButton>
						...
						<div class="w-full flex gap-2">
							...
							<div class="flex-1 flex items-center">
								<div
									...
									<span class="text-[#ffffff80]">{{
										getResolutionSize.width
									}}</span>
								</div>
								<div>x</div>
								<div
	...
	<div class="fixed right-4 top-[93px] z-[22]">
		<ElButton
			type="primary"
			class="!rounded-full w-[100px] !h-10"
			@click="openGoodsCaseImage">
			优秀案例
		</ElButton>
	</div>
	<ElImageViewer
...
const emit = defineEmits<{
	...
	(
		event: "generatePrompt",
		value: { promptId: number; prompt: string }
	): void;
}>();
...
const getTemplateOptions = computed(() => {
	...
	if (templates && templates.length > 0) {
		...
		return templates.filter(
			(item) => item.category_en === templateCateActive.value
		);
	}
	return [];
});
...
const handleResolutionChange = (value: any) => {
	formData.resolution = resolutionOptions.find(
...
defineExpose({
	getFormData: () => {
		return {
			...
			type_name: generateTypeTabs.find(
				(item) => item.id === generateType.value
			)?.name,
			style_name: styleOptions.find((item) => item.key === styleKey.value)
				?.label,
		};
	},
	validateForm: () => {
