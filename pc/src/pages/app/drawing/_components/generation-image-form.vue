<template>
			...
			<ElScrollbar>
				<div class="p-4">
					<ElForm :model="formData" label-position="top">
						...
						<ElFormItem
							label="提示词"
							v-if="formData.type === FormTypeEnum.TXT2IMAGE">
							<ElInput
							...
							<ElButton
								...
								@click="
									handleGeneratePrompt(
										CopywritingTypeEnum.AI_TEXT_TO_IMAGE
									)
								">
								生成提示词
							</ElButton>
						...
						<template
							...
							<ElFormItem>
								<div class="w-full">
									...
									<div
										...
										<div
											class="flex items-center gap-1 text-[11px] px-[6px]">
											<div
												v-for="(
													item, index
												) in imageTypeTabs"
							...
							<ElFormItem>
								...
								<ElButton
									...
									@click="
										handleGeneratePrompt(
											CopywritingTypeEnum.AI_IMAGE_TO_IMAGE
										)
									">
									生成提示词
								</ElButton>
	...
	<inspiration-image
		...
		@use-assemble="handleUseAssemble" />
...
const getModelChannel = computed(() => {
	appStore.getHdConfig.channel.forEach(
		(item) => (item.id = parseInt(item.id))
	);
...
defineExpose({
	getFormData: () => {
		...
		const data: any = {
			model,
			model_name: getModelChannel.value.find(
		...
		if (type == FormTypeEnum.TXT2IMAGE) {
			if (model == ModelEnum.HIDREAMAI) {
				...
				model == ModelEnum.GENERAL ||
				model == ModelEnum.SEEDREAM
			) {
