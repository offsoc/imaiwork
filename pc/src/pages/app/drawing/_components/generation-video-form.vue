			...
			<ElScrollbar>
				<div class="p-4">
					<ElForm :model="formData" label-position="top">
						...
						<ElFormItem
							v-if="
								formData.type == GenerateVideoTypeEnum.IMG2VIDEO
							">
							<template #label>
								<div
									class="flex items-center gap-2 text-[11px] px-[6px]">
									<div
							...
							<ElButton
								...
								@click="
									handleGeneratePrompt(
										CopywritingTypeEnum.AI_GENERATION_VIDEO
									)
								">
								生成提示词
							</ElButton>
...
defineExpose({
	getFormData: () => {
		...
		return {
			...
			type: formData.type,
			type_name:
				formData.type == GenerateVideoTypeEnum.TXT2VIDEO
					? "文生视频"
					: "图生视频",
			model: formData.model,
			model_name: getModelChannel.value.find(
				(item: any) => item.id == formData.model
			)?.name,
		};
	},
	validateForm: () => {
