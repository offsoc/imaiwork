<template>
    <div class="flex h-full w-full flex-col">
        <div class="grow flex min-h-0 gap-x-4">
            ...
            <div class="grow h-full min-h-0 relative bg-white flex flex-col py-4 min-w-[800px]" v-loading="loading">
                <div class="px-4 flex gap-x-4">
                    <div
                        ...
                        <Icon
                            ...
                            :color="previewType === item.type ? 'var(--color-primary)' : '#424242'"
                            :size="16"></Icon>
                        <span
                            class="text-lg"
                            :class="{
                ...
                <div class="w-full h-full grow min-h-0 flex flex-col mt-4">
                    <div class="h-full px-4" v-show="previewType === PreviewTypeEnum.RESULT">
                        <template v-if="taskResult.length > 0">
                            <div class="flex-1 flex flex-col h-full">
                                <ElScrollbar>
                                    <div class="flex flex-col gap-4 grow justify-center">
                                        <div v-for="result in taskResult">
                                            <div class="flex justify-between items-center">
                                                <div class="flex items-center gap-2">
                                                    <span class="tag">{{ result.params.template_name_zh }}</span>
                                                    <span class="tag">{{ result.params.resolution.join("*") }}</span>
                                                    <span class="tag">生成张数：{{ result.params.img_count }}</span>
                                                    <span class="tag">{{ generateEnumMap[result.params.style] }}</span>
                                                </div>
                                                ...
                                                <div class="grid grid-cols-4 gap-4">
                                                    <div
                                                        ...
                                                        <div class="absolute top-0 left-0 w-full h-full">
                                                            <image-container :item="item"></image-container>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div v-else class="flex justify-center items-center w-full h-full gap-x-6 mx-auto empty-box">
                            <template v-if="!imageUrl">
                                <img src="./_assets/images/goods_case.png" class="h-[290px]" />
                                <div class="shadow-[0_0_10px_rgba(0,0,0,0.1)] rounded-lg p-4 flex flex-col">
                                    <upload
                                        drag
                                        show-progress
                                        ...
                                        <div>
                                            <ElButton type="primary" size="large" :icon="Plus">上传图片</ElButton>
                                            <div class="w-[80%] text-xs mx-auto mt-2 text-[#545454] flex flex-col">
                                                <span>您也可以在此处上传商品图片</span>
                                                <span> 支持.jpg,.jpeg,.png文件 </span>
                                            </div>
                                        </div>
                                    </upload>
                                    <ElDivider class="!my-4">
                                        <span class="text-[#CFCFCF] text-xs font-normal">尝尝这些</span>
                                    </ElDivider>
                                    <div class="grid grid-cols-4 gap-4 grow">
                                        <div
                                            class="flex items-center gap-x-2 overflow-hidden cursor-pointer"
                                            v-for="item in caseLists.slice(0, 4)"
                                            :key="item"
                            ...
                            <div v-else class="shadow-[0_0_10px_rgba(0,0,0,0.1)] rounded-lg p-4 w-full">
                                <div
                                    class="w-full flex justify-center items-center border border-dashed border-token-primary-3 rounded-lg p-4 hover:border-primary relative">
                                    <img :src="imageUrl" class="h-[255px]" />
                                    <div class="absolute top-2 right-2">
                                        <ElButton :icon="Delete" @click="handleDeleteImage"></ElButton>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="w-full h-full grow min-h-0" v-if="previewType === PreviewTypeEnum.EXAMPLE">
                        <div v-if="caseLists.length > 0" class="h-full">
                            <ElScrollbar>
                                <div
                                    ...
                                    <div v-for="(data, index) in getColumnCaseLists" class="flex flex-col gap-y-4">
                                        <div
                                            v-for="item in data"
                                ...
                                <div v-if="caseFinished" class="flex justify-center items-center my-5">
                                    <span class="text-[#999999]">已加载全部</span>
                                </div>
                                <div v-if="caseLoading" class="flex items-center justify-center my-5">
                                    <span class="text-[#999999]">加载中</span>
                                    <Icon name="local-icon-loading3" color="#999999" :size="40"></Icon>
                                </div>
                            </ElScrollbar>
                        </div>
...
import { DrawTypeEnum, PreviewTypeEnum, FormGoodsTypeEnum, generateEnumMap } from "./_enums/drawEnums";
