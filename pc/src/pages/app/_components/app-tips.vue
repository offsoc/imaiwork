<template>
	<div
		class="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-[#0000009c] z-[9999]"
		v-if="show">
		<div class="relative w-[502px]">
			<template v-if="!loading">
				<img
					src="@/assets/images/app_tips_bg.png"
					class="w-full h-[582px]" />
				<img
					src="@/assets/images/close.png"
					class="w-12 h-12 mt-6 mx-auto cursor-pointer"
					@click="close" />
				<div
					class="absolute top-[46px] left-[38px] text-[48px] text-white font-bold">
					{{ name }}
				</div>
				<div
					class="absolute top-[200px] w-full flex flex-col items-center justify-center">
			...
			<div
				v-else
				class="w-full h-[500px] bg-white flex items-center justify-center rounded-lg">
				<Loader />
			</div>
		</div>
	</div>
...
import { getWechatQrcode } from "@/api/app";
