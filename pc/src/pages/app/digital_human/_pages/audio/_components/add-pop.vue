...
const getHumanAudioToken = userStore.getTokenByScene(
	TokensSceneEnum.HUMAN_AUDIO
)?.score;
const getHumanAudioProToken = userStore.getTokenByScene(
	TokensSceneEnum.HUMAN_AUDIO_PRO
)?.score;
...
const getToneList = async () => {
	...
	toneLists.value = lists.length
		? lists.filter((item: any) => item.status == 1)
		: [];
};
