			...
			<DynamicScroller
				...
				<template #default="{ item, index, active }">
					<DynamicScrollerItem
						:item="item"
						:active="active"
						:size-dependencies="[item.friends]">
						<div class="w-full h-full flex flex-col">
							...
							<div
								...
								<div class="flex items-center h-[56px]">
									...
									<span>{{
										friend.Remark || friend.FriendNick
									}}</span>
								</div>
							</div>
...
const filteredFriends = computed(() => {
	...
	props.friendList.forEach((friend) => {
		...
		if (friend.FriendNick?.includes(search.value))
...
const handleClickFriend = (friend: any) => {
	if (
		currentFriend.value?.UserName === friend.FriendId ||
		currentWechat.value.wechat_id === friend.FriendId
	)
		return;
	currentFriend.value = {
