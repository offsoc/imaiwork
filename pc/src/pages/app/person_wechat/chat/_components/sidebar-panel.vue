<template>
	<div class="w-full h-full bg-black flex flex-col py-2">
			...
			<ElScrollbar>
				<div class="flex flex-col gap-y-1 mt-4 px-2">
					<div
						...
						:class="{
							'bg-[#00C800]':
								currentWechat?.device_code === item.device_code,
							'hover:bg-[#00C800]': !item.loading,
						}"
						@click="handleSelectWechat(item)">
						<ElTooltip
							:content="item.wechat_nickname"
							placement="right">
							<div class="relative w-[48px] h-[48px] rounded">
								...
								<div
									...
									<Icon
										name="local-icon-offline"
										:size="24"></Icon>
								</div>
								<div
									class="absolute top-0 left-0 w-full h-full bg-black/5 z-[888]"
									v-if="item.loading">
									<div
										class="w-full h-full rounded bg-gray-200 animate-pulse"></div>
								</div>
							</div>
						</ElTooltip>
					</div>
		...
		<div class="flex justify-center mt-2 mx-2">
			<ElButton color="#00C800" class="w-full" @click="handleAddWeChat">
				添加微信
			</ElButton>
		</div>
	</div>
