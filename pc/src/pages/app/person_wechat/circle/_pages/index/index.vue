<template>
	<div class="w-full h-full">
		<div class="h-full flex gap-x-4">
			<div
				class="flex-1 flex rounded-xl overflow-hidden"
				ref="containerRef">
				...
				<div class="flex-1 flex flex-col">
					<div
						class="flex justify-between gap-x-4 h-[48px] items-center bg-[#EDEDED] px-4 flex-shrink-0">
						<ElAlert type="warning" :closable="false">
							<div>
								朋友圈更新存在一定延迟，如有数据不同步，请多刷新
							</div>
						</ElAlert>
						<ElButton
							link
						...
						<div
							...
							<div class="flex flex-col items-center">
								<Icon
									...
									:size="50"
									color="#ffffff"></Icon>
								<span class="text-white text-2xl mt-2"
									>设备离线</span
								>
							</div>
						</div>
			...
			<div class="w-[462px] rounded-xl overflow-hidden">
				<CircleSend
					v-model="circleSendForm"
					:is-show-we-chat="false"
					@success="refreshCircle()" />
			</div>
		</div>
	</div>
	<ElImageViewer
...
function handleCirclePushNotice(Content: any) {
	...
	if (Circles && Circles.length) {
		...
		if (circleList.value.length) {
			if (
				circleList.value[circleList.value.length - 1].CircleId ==
				Circles[0].CircleId
			) {
				Circles.shift();
			}
	...
	const handlers: Record<MsgTypeEnum, Function> = {
		...
		[MsgTypeEnum.DeleteSNSNewsTask]: () => {
			circleList.value = circleList.value.filter(
				(item) => item.CircleId != circleId.value
			);
			circleId.value = null;
		},
				...
				if (item.CircleId == circleId.value) {
					if (isLiked.value) {
						item.Likes = item.Likes.filter(
							(like) => like.FriendId != Content.WeChatId
						);
					} else {
						item.Likes.push({
							CircleId: "0",
			...
			circleList.value.forEach((item) => {
				if (item.CircleId == circleId.value) {
					item.Comments = item.Comments.filter(
						(comment) => comment.CommentId != Content.CommentId
					);
				}
			});
		},
...
on("error", async (data: any) => {
	...
	if (
		MsgType == MsgTypeEnum.CircleCommentReplyTaskResultNotice ||
		MsgType == MsgTypeEnum.CircleCommentReplyTask
	) {
		const { CircleId } = Content;
		circleList.value.forEach((item) => {
			if (item.CircleId == CircleId) {
				item.Comments = item.Comments.filter(
					(comment) => comment.SendType != "web"
				);
			}
		});
	}
