<template>
    <div class="h-full flex flex-col bg-white rounded-xl" v-if="!isCreate && !isRecord">
        <div class="flex items-center justify-between p-4">
            <ElButton type="primary" @click="handleCreate()">添加自动SOP</ElButton>
        ...
        <div class="grow min-h-0 flex flex-col">
            <div class="grow min-h-0">
                <ElTable
                    ...
                    <ElTableColumn label="营销总天数" prop="all_day" width="100">
                        <template #default="{ row }"> {{ row.all_day }}天 </template>
                    </ElTableColumn>
                    <ElTableColumn prop="type" label="营销类型" min-width="120">
                        <template #default="{ row }">
                            {{ SendWayMap[row.type] }}
                        </template>
                    </ElTableColumn>
                    <ElTableColumn label="任务状态" width="220px">
                        <template #default="{ row }">
                    ...
                    <ElTableColumn label="创建时间" prop="create_time" width="200px"> </ElTableColumn>
...
import { SidebarTypeEnum, PushTypeEnum, SendWayMap } from "../../_enums";
...
const isCreate = ref(query.is_create == "1" && parseInt(query.type as string) == SidebarTypeEnum.AUTO_SOP);
const isRecord = ref(query.is_record == "1" && parseInt(query.type as string) == SidebarTypeEnum.AUTO_SOP);
...
const back = () => {
    ...
    window.history.replaceState("", "", `?type=${SidebarTypeEnum.AUTO_SOP}`);
    getLists();
};
