<template>
	<div
		class="h-full flex flex-col bg-app-bg-2 rounded-[20px]"
...
const isCreate = ref(
	query.is_create == "1" &&
		parseInt(query.type as string) == SidebarTypeEnum.AUTO_GET_CUSTOMER
);
const handleCreate = () => {
	isCreate.value = true;
	replaceState({
...
const handleRetry = (row: any) => {
	nuxtApp.$confirm({
		title: "任务异常情况",
		message:
			"提醒：重新启动只会启动异常设备<br /><br />1. 设备启动未执行任务<br />2. 设备断开重连<br />",
