<template>
	<div class="flex h-screen w-full flex-col items-center">
		...
		<div
			class="relative flex w-full grow flex-col lg:flex-row overflow-hidden">
			<div
				class="w-full flex-1 lg:flex-auto lg:w-1/2 justify-center bg-white min-h-0">
								...
								<upload
									...
									<button class="h-20 w-20 mt-10 relative">
										...
										<div
											...
											<Icon
												name="local-icon-loading2"
												custom-class="animate-spin"></Icon>
										</div>
									</button>
								</upload>
							</div>
									...
									<ElFormItem label="关联知识库">
										<ElSelect
											...
											<ElOption
												v-for="(
													item, index
												) in optionsData.knowledgeBaseList"
												:label="item.name"
									...
									<ElFormItem label="填写表单">
										<div class="w-full">
											...
											<ElTable
												...
												<ElTableColumn
													label="输入长度"
													prop="props[maxlength]"></ElTableColumn>
										...
										<div
											...
											<template
												...
												<button
													...
													<div
														class="line-clamp-3 text-balance text-gray-600 break-all">
														{{ item.value }}
													</div>
												</button>
											</template>
...
const inputDol = (e: any, index: number) => {
	...
	// 判断 preliminary_ask
	if (
		formData.preliminary_ask.length === 1 &&
		!formData.preliminary_ask[0].value
	)
		return;
	if (
		formData.preliminary_ask[index].value &&
		formData.preliminary_ask[laseIndex].value != ""
	) {
...
const insertAFormField = (field: string) => {
	formData.form_info = setRangeText(
...
const handleFormAdd = useThrottleFn((value: any) => {
	const isSameField = !!formData.template_info.form.find(
		(item: any) => item.props.field === value.props.field
	);
	const isSameType = !!formData.template_info.form.find(
		(item: any) => item.name === "WidgetFile"
	);
	if (isSameType) {
...
const handleDeleteForm = async (index: number) => {
	...
	formData.form_info = replaceDynamicString(
		formData.form_info,
		currFiledValue
	);
