<template>
        ...
        <div class="grow min-h-0">
            <ElTable
                ...
                <ElTableColumn label="操作" width="200">
                    <template #default="{ row }">
                        <ElButton type="primary" link @click="handleCode(row)"> 查看代码 </ElButton>
                        <!-- 更多操作 -->
    ...
    <usage-setting
        ...
        @success="getLists" />
    <code-view v-if="showCode" ref="codeViewRef" @close="showCode = false" />
</template>
...
const codeViewRef = shallowRef<InstanceType<typeof CodeView>>();
const handleCode = async (row: any) => {
    showCode.value = true;
    await nextTick();
    codeViewRef.value?.open();
    codeViewRef.value?.setFormData(row);
};
