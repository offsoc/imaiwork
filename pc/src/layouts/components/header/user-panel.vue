<template>
	<ElPopover
		trigger="click"
		width="256px"
		popper-class="!border-none !rounded-2xl !p-0"
		:show-arrow="false">
		<template #reference>
		...
		<div class="px-6 py-[18px]">
			<div class="flex flex-col gap-y-2 text-xs">
				<div class="flex items-center gap-x-2">
					用户名：<span class="text-[#00000080]">{{
						userInfo.nickname
					}}</span>
					<ElTooltip content="复制">
						<span
							class="cursor-pointer"
							@click="copy(userInfo.nickname)">
				...
				<div>
					注册时间：<span class="text-[#00000080]">{{
						dayjs(userInfo.created_at).format("YYYY-MM-DD")
					}}</span>
				</div>
			</div>
			<div class="my-4">
	...
	<base-popup
		v-if="showBasePop"
		ref="basePopupRef"
		@close="showBasePop = false"></base-popup>
</template>
