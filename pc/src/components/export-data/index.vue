<template>
    <div class="export-data">
        <popup
            ...
            <div>
                <ElForm ref="formRef" :model="formData" label-width="120px" :rule="formRules">
                    <ElFormItem label="数据量：">
                        预计导出{{ exportData.count }}条数据， 共{{ exportData.sum_page }}页，每页{{
                            exportData.page_size
                        }}条数据
                    </ElFormItem>
                    <ElFormItem label="导出限制：">
                        每次导出最大允许{{ exportData.max_page }}页，共{{ exportData.all_max_size }}条数据
                    </ElFormItem>
                    <ElFormItem prop="page_type" label="导出范围：" required>
                        <ElRadioGroup v-model="formData.page_type">
                            <ElRadio :label="0">全部导出</ElRadio>
                            <ElRadio :label="1">分页导出</ElRadio>
                        </ElRadioGroup>
                    </ElFormItem>
                    <ElFormItem label="分页范围：" v-if="formData.page_type == 1">
                        <div class="flex">
                            <ElFormItem prop="page_start">
                                <ElInput
                                    style="width: 140px"
                    ...
                    <ElFormItem label="导出文件名称：" prop="file_name">
                        <ElInput v-model="formData.file_name" placeholder="请输入导出文件名称"></ElInput>
                    </ElFormItem>
                </ElForm>
            </div>
        </popup>
